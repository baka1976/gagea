...
`?(n.push(s.trim()),t.push(n),n=[],s=""):r==="\r"||(s+=r)}return(s||n.length)&&(n.push(s.trim()),t.push(n)),t.filter(o=>o.some(r=>r!==""))}var me={date:["거래일시","거래일자","이용일자","거래일","이용일","승인일","매출일","날짜","일자"],amount:["이용금액","승인금액","거래금액","결제금액","금액"],merchant:["가맹점명","기재내용","가맹점","거래처","적요","내용","상호","내역","비고","받는분","보낸분"],balance:["거래후잔액","거래후 잔액","잔액","잔고"],outCol:["출금금액","출금액","출금 (원)","출금(원)","찾으신금액","찾으신 금액","지급금액","출금","지급"],inCol:["입금금액","입금액","입금 (원)","입금(원)","맡기신금액","맡기신 금액","예입금액","입금","예입"],installment:["이용구분","할부구분","할부개월"],cancel:["취소상태","매입구분","승인상태"],cardName:["이용카드","카드명","카드구분"],kind:["거래구분","거래종류","거래유형","적요","구분"]},ri=/(총\s*\d+\s*건|출금건수|입금건수|합\s*계|소\s*계|^\d+\/\d+$)/,Je=e=>String(e||"").replace(/\s+/g,"");function ea(e){let t=-1,n=null;for(let r=0;r<Math.min(e.length,25);r++){let l=(e[r]||[]).map(Je),d=l.some(u=>u&&me.date.some(c=>u.includes(Je(c)))),f=l.some(u=>u&&(me.amount.some(c=>u.includes(Je(c)))||me.outCol.some(c=>u.includes(Je(c)))||me.inCol.some(c=>u.includes(Je(c)))));if(d&&f){t=r,n=l;break}}if(t===-1)return null;let s=(r,l)=>{for(let d of r){let f=Je(d);for(let u=0;u<n.length;u++)if(!(l&&l.includes(u))&&n[u]===f)return u}for(let d of r){let f=Je(d);for(let u=0;u<n.length;u++)if(!(l&&l.includes(u))&&n[u]&&n[u].includes(f)&&!n[u].startsWith("해외"))return u}return-1},i=s(me.outCol),a=s(me.inCol,i>=0?[i]:[]),o=i>=0&&a>=0&&i!==a;return{headerRow:t,header:n,colDate:s(me.date),colAmount:o?-1:s(me.amount),colOut:o?i:-1,colIn:o?a:-1,colMerchant:s(me.merchant),colBalance:s(me.balance),colInstallment:s(me.installment),colCancel:s(me.cancel),colKind:s(me.kind)}}var Ot=e=>{let t=parseInt(String(e||"").replace(/[^\d-]/g,""),10);return isNaN(t)?0:t};function kn(e){let t=String(e||"").replace(/[\n\r]+/g," ").trim(),n=t.match(/(20\d{2})[.\-/년\s]+(\d{1,2})[.\-/월\s]+(\d{1,2})/);return n?le(new Date(+n[1],+n[2]-1,+n[3])):(n=t.match(/(\d{2})[.\-/](\d{1,2})[.\-/](\d{1,2})/),n?le(new Date(2e3+ +n[1],+n[2]-1,+n[3])):null)}var li=["네이버페이","카카오페이","비바리퍼블리카","토스페이","페이코","쿠페이","스마일페이","삼성페이","애플페이","제로페이","페이머니","머니충전"];function ta(e){let t=String(e||"").replace(/\s+/g,"").toLowerCase();return li.some(n=>t.includes(n.toLowerCase()))}function na(e,t,n={}){let s=new Map,i=r=>{let l=(s.get(r)||0)+1;return s.set(r,l),l},a=[],o=[];for(let r=t.headerRow+1;r<e.length;r++){let l=e[r]||[],d=l.join(" ");if(ri.test(d.replace(/\s+/g," ").trim())){o.push(r);continue}let f=kn(l[t.colDate]);if(!f){o.push(r);continue}let u=0,c=null,h=!1;if(t.colOut>=0&&t.colIn>=0){let N=Ot(l[t.colOut]),W=Ot(l[t.colIn]);N>0?(u=N,c="out"):W>0&&(u=W,c="in")}else{let N=Ot(l[t.colAmount]);h=N<0,u=Math.abs(N),c=n.defaultDirection||"out"}if(!u){o.push(r);continue}let g=(t.colMerchant>=0?String(l[t.colMerchant]||""):"").trim();if(/^[\d-]+$/.test(g.replace(/\s/g,""))&&t.colKind>=0){let N=String(l[t.colKind]||"").trim();N&&(g=N)}let m=(t.colKind>=0?String(l[t.colKind]||""):"").trim(),y=m&&m!==g?`${m} ${g}`:g,v=t.colBalance>=0?Ot(l[t.colBalance]):null,$=t.colCancel>=0?String(l[t.colCancel]||""):"",_=t.colInstallment>=0?String(l[t.colInstallment]||""):"",A=h||/(취소|환불)/.test($)||/(취소|환불)/.test(g),w=_.match(/할부\s*\((\d{1,2})\s*개월\)/)||_.match(/(\d{1,2})\s*개월/),D=/해외/.test(_),x=c==="in"?"transfer":"expense",M=null,B=!1,H=null,j=!1;if(c==="in"){let N=Ye({counterpartyRaw:g,memo:g,rawText:g,amount:u},n.settings);N?(x=N.type,M=N.transferKind,B=!!N.needsCheck,N.isRefund&&(j=!0),N.confidence!=null&&(H=N.confidence)):/(급여|월급|상여|환급)/.test(g)&&(x="income")}let X=x==="income"?"income":ta(g)&&!Ne(z(y),n.rules)&&!Ne(z(g),n.rules)&&!(n.settings&&(n.settings.familyTransferKeywords||[]).some(N=>N&&g.includes(N)))?"etc":be(y,"expense",n.rules),J=Pe({counterpartyRaw:g,memo:g,rawText:d,category:X,amount:u},n.settings);J&&J.transferKind&&J.transferKind!=="welfare"&&!J.category&&(x="transfer");let I=!1;J&&(J.type&&(x=J.type),J.category&&(X=J.category),M=J.transferKind,I=!!J.isExtra,B=!1,J.confidence!=null&&(H=J.confidence)),M&&["internal","invest","savings","cashOut","cardPayment","fxTopup","verify"].includes(M)&&(B=!1,H==null&&(H=1)),a.push({type:x,direction:c,date:f,amount:u,transferKind:M,needsCheck:B,isExtra:I,category:X,memo:g||"내역",normKey:z(g),counterpartyRaw:g,balanceAfter:v||null,accountId:n.accountId||null,paymentMethod:n.paymentMethod||"account",source:"statement",isRefund:A||j,installmentMonths:w?parseInt(w[1],10):null,isOverseas:D,confidence:H??(X==="etc"?.4:.8),needsCheck:A||j?!1:B,sourceHash:`${n.accountId||""}|${f}|${u}|${g}|${$}|${c}|${v||""}|${i(`${f}|${u}|${g}|${c}|${v||""}`)}`,rawText:d.trim()})}return{entries:a,skipped:o}}function sa(e){let t=new TextDecoder("utf-8",{fatal:!1}).decode(e);if((t.match(/\uFFFD/g)||[]).length>3)try{return new TextDecoder("euc-kr").decode(e)}catch{}return t}var bn=(e,t)=>le(new Date(new Date(e+"T00:00:00").getTime()+t*wn));function xn(e,t){let n=e.filter(h=>!h.deleted&&h.tripId===t.id),s=0,i=0,a=0,o=0,r={},l=[];for(let h of n){if(h.date&&l.push(h.date),h.type==="income"){i+=h.amount;continue}if(h.type!=="expense")continue;let g=h.isRefund?-h.amount:h.amount;if(h.transferKind==="welfare"){a+=g;continue}h.transferKind&&h.transferKind!=="external"||(h.reimbFrom&&(o+=h.amount),s+=g,r[h.category]=(r[h.category]||0)+g)}l.sort();let d=Object.entries(r).sort((h,g)=>g[1]-h[1]),f={};for(let h of n)h.currency&&h.foreignAmount&&(f[h.currency]=(f[h.currency]||0)+h.foreignAmount);let u=t.startDate||l[0]||null,c=t.endDate||l[l.length-1]||null;return{count:n.length,total:s,income:i,welfare:a,reimbursed:o,cats:d,fx:f,from:u,to:c,days:u&&c?Math.round((new Date(c)-new Date(u))/wn)+1:null}}var ci=["anthropic","openai","oracle","google","apple.com","netflix","spotify","aws","amazon web","microsoft","github","adobe","cloudflare","steam","paypal","patreon","figma","notion","dropbox"];function aa(e){if(!e||e.deleted||e.type!=="expense")return!1;let t=String(e.memo||"").toLowerCase();return ci.some(n=>t.includes(n))?!1:!!(e.currency&&e.currency!=="KRW"||e.isOverseas)}function oa(e,t={}){let n=t.gapDays||3,s=e.filter(aa).sort((l,d)=>l.date<d.date?-1:1);if(!s.length)return{trips:[],singles:[]};let i=[],a=[s[0]];for(let l=1;l<s.length;l++)_e(a[a.length-1].date,s[l].date)<=n?a.push(s[l]):(i.push(a),a=[s[l]]);i.push(a);let o=[],r=[];for(let l of i){let d=l[0].date,f=l[l.length-1].date,u=_e(d,f)+1,c=l.reduce((m,y)=>m+(y.isRefund?-y.amount:y.amount),0),h=[...new Set(l.map(m=>m.currency).filter(Boolean))],g={startDate:d,endDate:f,days:u,count:l.length,localSpent:c,currencies:h,ids:l.map(m=>m.id),name:ui(l,d)};l.length>=3||u>=2?o.push(g):r.push(g)}return{trips:o.reverse(),singles:r.reverse()}}var di=[["삿포로",["sapporo","satsuporo","hakodate","chitose","moiwa","seicomart","yufutsu"]],["오키나와",["okinawa","naha","miyako","kokusai","coral port"]],["도쿄",["tokyo","shinjuku","shibuya","haneda","narita","asakusa"]],["오사카",["osaka","namba","umeda","kansai","dotonbori"]],["후쿠오카",["fukuoka","hakata","tenjin"]],["베트남",["vietnam","hanoi","saigon","danang","marou"]],["태국",["thailand","bangkok","phuket"]]];function ui(e,t){let n=e.map(a=>String(a.memo||"")).join(" ").toLowerCase();for(let[a,o]of di)if(o.some(r=>n.includes(r)))return`${a} ${t.slice(2,7).replace("-",".")}`;let s=e.find(a=>a.currency)?.currency;return`${{JPY:"일본",VND:"베트남",THB:"태국",USD:"미국",EUR:"유럽",TWD:"대만"}[s]||"여행"} ${t.slice(2,7).replace("-",".")}`}function pi(e,t){if(e.direction!=="in"||e.type!=="transfer"||!(e.amount>0)||e.transferKind&&e.transferKind!=="excluded")return!1;let n=t.settings||{},s=`${e.memo||""} ${e.counterpartyRaw||""}`;return!(n.userName&&s.includes(n.userName)||(n.excludedCounterparties||[]).some(i=>i&&s.includes(i)))}function Ut(e,t,n={}){if(!t||!t.startDate||!t.endDate)return{...t||{},inside:[],booked:[],reimb:[],onsite:0,prepaid:0,total:0,welfare:0};let s=n.preDays??120,i=n.postDays??0,a=(n.allTrips||[]).filter(m=>m.startDate&&m.startDate!==t.startDate&&m.id!==t.id),o=bn(t.startDate,-s),r=bn(t.endDate,i),l=bn(t.endDate,Math.max(i,60)),d=[],f=[],u=[];for(let m of e){if(m.deleted||m.tripId&&t.id&&m.tripId!==t.id)continue;if(pi(m,n)){m.date>=o&&m.date<=l&&u.push(m);continue}if(m.type!=="expense"||m.fixedId||m.category==="family")continue;let y=m.transferKind==="welfare";if(m.transferKind&&m.transferKind!=="external"&&!y)continue;let v=y?{...m,welfare:!0}:m;if(m.date>=t.startDate&&m.date<=t.endDate){d.push(v);continue}let $=m.date>=o&&m.date<t.startDate,_=i>0&&m.date>t.endDate&&m.date<=r;if(!$&&!_||$&&a.some(w=>w.startDate>m.date&&w.startDate<t.startDate)||_&&a.some(w=>w.startDate>t.endDate&&w.startDate<=m.date))continue;let A=$?"pre":"post";m.category==="travel"||t.id&&m.tripId===t.id?f.push({...v,sure:!0,when:A}):m.isOverseas||m.currency&&m.currency!=="KRW"?f.push({...v,sure:!1,when:A}):n.wide&&f.push({...v,sure:!1,when:A})}let c=m=>m.filter(y=>!y.welfare).reduce((y,v)=>y+(v.isRefund?-v.amount:v.amount),0),h=(m,y)=>m.date<y.date?-1:1,g=f.filter(m=>m.sure);return{...t,inside:d.sort(h),booked:f.sort(h),reimb:u.sort(h),onsite:c(d),prepaid:c(g),total:c(d)+c(g),welfare:[...d,...g].filter(m=>m.welfare).reduce((m,y)=>m+(y.isRefund?-y.amount:y.amount),0)}}function ia(e,t={}){let n=t.days||10,s=d=>d.transferKind&&d.transferKind!=="excluded",i=d=>!d.deleted&&d.amount>0&&!s(d)&&d.category!=="family"&&d.type!=="income",a=e.filter(d=>i(d)&&d.direction==="out"&&!d.transferKind),o=e.filter(d=>i(d)&&d.direction==="in"&&!d.isRefund),r=new Set,l=[];for(let d of o){let f=d.normKey||z(d.memo),u=null;for(let c of a){if(r.has(c.id)||c.amount!==d.amount)continue;let h=_e(c.date,d.date);if(h<0||h>n)continue;let g=c.normKey||z(c.memo);f&&g&&(f===g||f.includes(g)||g.includes(f)||Ge(f,g)>=.55)&&(!u||_e(c.date,d.date)<_e(u.date,d.date))&&(u=c)}u&&(r.add(u.id),l.push({refund:d,original:u}))}return l}var fi=[["엔진오일",["엔진오일","엔진 오일","engine oil","모빌","킥스","지크"]],["오일필터",["오일필터","오일 필터","오일휠타","오일 휠타","oil filter"]],["에어필터",["에어필터","에어 필터","에어클리너","흡기필터","air filter"]],["에어컨필터",["에어컨필터","에어컨 필터","캐빈필터","실내필터","향균필터"]],["미션오일",["미션오일","미션 오일","변속기오일","atf","cvt오일","오토미션"]],["점화플러그",["점화플러그","점화 플러그","스파크플러그","플러그","spark plug"]],["브레이크패드",["브레이크패드","브레이크 패드","패드","제동패드","brake pad"]],["브레이크액",["브레이크액","브레이크 오일","브레이크오일","제동액","brake fluid"]],["냉각수",["냉각수","부동액","쿨란트","coolant"]],["타이어",["타이어","tire","tyre","휠얼라인먼트","얼라인먼트"]],["배터리",["배터리","밧데리","축전지","battery"]],["와이퍼",["와이퍼","wiper","블레이드"]],["연료필터",["연료필터","연료 필터","퓨얼필터","fuel filter"]],["LPG 필터",["lpg필터","lpg 필터","가스필터"]],["DPF 점검",["dpf","디피에프","매연포집"]],["감속기 오일",["감속기","리덕션","reduction"]]],mi=/(합계|총액|부가세|공급가|할인|소계|청구|결제|카드|현금|영수증|견적|사업자|대표|전화|주소|고객|차량번호|담당)/;function ra(e,t={}){let n=String(e||"").split(/\n/).map(r=>r.trim()).filter(Boolean),s=[],i=null,a=null,o=0;for(let r of n){if(!i){let c=r.match(/(20\d{2})[.\-/년\s]+(\d{1,2})[.\-/월\s]+(\d{1,2})/);c&&(i=le(new Date(+c[1],+c[2]-1,+c[3])))}if(a==null){let c=r.match(/([0-9][0-9,]{2,})\s*(km|KM|키로|주행)/);if(c)a=parseInt(c[1].replace(/,/g,""),10);else if(/주행|계기|누적/.test(r)){let h=r.match(/([0-9][0-9,]{3,})/);h&&(a=parseInt(h[1].replace(/,/g,""),10))}}let l=r.toLowerCase(),d=r.match(/([0-9]{1,3}(?:,[0-9]{3})+|[0-9]{4,})\s*원?/),f=d?parseInt(d[1].replace(/,/g,""),10):0;if(mi.test(r)){/(합계|총액|청구)/.test(r)&&f>o&&(o=f);continue}let u=fi.find(([,c])=>c.some(h=>l.includes(h)));u&&(s.some(c=>c.label===u[0])||s.push({label:u[0],amount:f,raw:r}))}return o||(o=s.reduce((r,l)=>r+l.amount,0)),{items:s,date:i||oe(),mileage:a,total:o}}function Cn(e){return!e||e.deleted||e.type!=="expense"||e.category==="family"?!1:!e.transferKind||["external","welfare","verify"].includes(e.transferKind)}function Dn(e,t){if(!Cn(e)||e.catBy==="user")return e;let n=Ne(e.normKey||z(e.memo),t);if(!n)return e;let s={...e,category:n.category||e.category,confidence:1};return n.kind==="exclude"&&e.transferKind!=="welfare"&&(s.transferKind="verify"),s.status==="pending"&&!e.needsCheck&&(s.status="confirmed"),s}function hi(e,t){if(e=String(e||""),t=String(t||""),!e||!t||e===t)return!1;let[n,s]=e.length<=t.length?[e,t]:[t,e];if(n.length>=2&&s.startsWith(n)||n.length>=3&&s.includes(n))return!0;let i=0;for(;i<n.length&&n[i]===s[i];)i++;return i>=4&&i/n.length>=.6?!0:n.length>=4&&Ge(e,t)>=.6}function la(e,t,n){let s=t&&(t.normKey||z(t.memo))||"",i=[],a=[];if(s.length<2)return{same:i,similar:a};for(let o of e){if(!Cn(o)||o.id===t.id||o.category===n||o.catBy==="user")continue;let r=o.normKey||z(o.memo);!r||r.length<2||(r===s?i.push(o):hi(r,s)&&(o.category==="etc"||o.category===t.category)&&a.push(o))}return{same:i,similar:a}}var gi=[["food",["식비","식사","음식","식당","장보기"]],["cafe",["카페","간식","커피"]],["transport",["교통"]],["car",["차량","차","주유","정비","유지비","자동차"]],["shopping",["쇼핑"]],["living",["주거","생활"]],["health",["의료","건강","병원"]],["culture",["문화","여가"]],["comm",["통신","인터넷"]],["insurance",["보험"]],["loan",["대출","이자"]],["family",["가족"]],["social",["경조","축의","부의","회비","모임"]],["travel",["여행"]],["etc",["기타"]]];function ca(e){let t=[];for(let n of String(e||"").split(/\n/)){let s=n.replace(/#.*/,"").trim();if(!s)continue;let i=s.match(/^(.+?)\s*(?:=|→|->|:)\s*(.+)$/);if(!i){t.push({line:s,error:"'이름 = 분류' 형태로 적어주세요"});continue}let a=i[1].trim(),o=i[2].replace(/\s+/g,""),r=z(a);if(r.length<2){t.push({line:s,error:"이름이 너무 짧아요"});continue}if(/제외|확인용|빼기/.test(o)){t.push({line:s,name:a,normKey:r,category:"etc",kind:"exclude"});continue}let l=gi.find(([,d])=>d.some(f=>o.includes(f)))||Se.filter(d=>d.key!=="income").map(d=>[d.key,[d.label.replace(/\s/g,"")]]).find(([,d])=>d.some(f=>o.includes(f)));if(!l){t.push({line:s,error:`'${i[2].trim()}'는 없는 분류예요`});continue}t.push({line:s,name:a,normKey:r,category:l[0],kind:null})}return t}function jt(e,t){let n=new Map,s=new Map(t.map(i=>[i.normKey,0]));for(let i of e){if(!Cn(i))continue;let a=Ne(i.normKey||z(i.memo),t);if(!a)continue;let o={category:a.category};a.kind==="exclude"&&i.transferKind!=="welfare"&&(o.transferKind="verify"),(i.category!==o.category||o.transferKind&&i.transferKind!==o.transferKind)&&(n.set(i.id,o),s.set(a.normKey,(s.get(a.normKey)||0)+1))}return{changes:n,perRule:s}}var $i=/^(우리|신한|국민|농협|하나|기업|전북|광주|부산|대구|경남|제주|산업|카카오|토스|케이|새마을|신협|우체국|SC|IBK|KB|NH)\s*/;function vi(e,t){let n=String(e||"").trim().replace($i,"").replace(/\s+/g,"");return!(!/^[가-힣]{2,4}$/.test(n)||t&&n===String(t).replace(/\s+/g,"")||/(마트|식당|상회|약국|의원|카페|슈퍼|문구|치킨|분식|정육|세탁|학원|병원|철물|주유|전자|상사|농협|은행|카드|페이|머니|충전)$/.test(n))}function yi(e,t={}){return!e.length||!e.every(n=>n.category==="etc"&&n.paymentMethod!=="card")||!vi(e[0].memo,t.userName)?!1:e.every(n=>n.amount>=3e4&&n.amount<=1e6&&n.amount%1e4===0)}function da(e,t={}){if(!e||e.deleted||e.source!=="shortcut"||e.localized||Number(e.updatedAt||0)!==Number(e.createdAt||0))return null;let n=String(e.rawText||"");if(!n.trim())return null;let s=new Date(Number(e.createdAt)||Date.now()),i=Lt(n,{rules:t.rules,settings:t.settings,now:s});if(!i||!i.amount)return null;let a={type:i.type,category:i.category,transferKind:i.transferKind||null,isRefund:!!i.isRefund,isExtra:!!i.isExtra,welfareBalance:i.welfareBalance??null,confidence:i.confidence,localized:!0};return i.direction&&(a.direction=i.direction),i.balanceAfter!=null&&(a.balanceAfter=i.balanceAfter),Tt(n,t.settings)!=="free"&&(a.memo=i.memo,a.date=i.date,a.normKey=i.normKey,i.counterpartyRaw&&(a.counterpartyRaw=i.counterpartyRaw)),a.needsCheck=!!i.needsCheck,a.status=i.needsCheck||(i.confidence??1)<.6?"pending":"confirmed",a}function Sn(e,t){if(!e||e.tripId||!aa(e))return null;let n=(t||[]).find(s=>s.startDate&&s.endDate&&e.date>=s.startDate&&e.date<=s.endDate);return n?n.id:null}function ua(e,t={}){let n=t.settings&&t.settings.travelKeywords||[];if(!n.length)return[];let s=t.gapDays||4,i=e.filter(r=>!r.deleted&&r.type==="expense"&&!r.tripId&&!r.isRefund&&n.some(l=>l&&`${r.memo||""} ${r.counterpartyRaw||""}`.includes(l))).sort((r,l)=>r.date<l.date?-1:1),a=[],o=[];for(let r of i)o.length&&_e(o[o.length-1].date,r.date)>s&&(a.push(o),o=[]),o.push(r);return o.length&&a.push(o),a.filter(r=>r.length>=2).map(r=>({startDate:r[0].date,endDate:r[r.length-1].date,approx:!0,days:_e(r[0].date,r[r.length-1].date)+1,count:r.length,localSpent:r.reduce((l,d)=>l+d.amount,0),currencies:[],ids:r.map(l=>l.id),name:`여행 ${r[0].date.slice(2,7).replace("-",".")}`})).reverse()}function pa(e,t=[]){let n=a=>typeof a=="string"&&/^\d{4}-\d{2}-\d{2}$/.test(a),s=new Set,i=[];for(let a of e||[]){if(!a||!n(a.start))continue;let o=a.start,r=n(a.end)&&a.end>=a.start?a.end:a.start,l=`${o}|${r}`;if(s.has(l)||(s.add(l),(t||[]).some(u=>u.startDate&&u.endDate&&u.startDate<=r&&u.endDate>=o)))continue;let f=String(a.title||"").replace(/\s+/g," ").trim().slice(0,20)||`여행 ${o.slice(2,7).replace("-",".")}`;i.push({name:f,startDate:o,endDate:r,days:_e(o,r)+1,fromCalendar:!0,currencies:[]})}return i.sort((a,o)=>a.startDate<o.startDate?1:-1)}var Ft={date:/날짜|date/i,time:/시간|time/i,kind:/종류|payment\s*type/i,currency:/통화|currency/i,amount:/^금액|amount(?!.*krw)/i,balance:/잔액|balance/i,merchant:/가맹점|merchant/i,approval:/승인번호|approval/i,rate:/환율|exchange/i,krw:/원화금액|krw/i};function Wt(e){if(!Array.isArray(e))return!1;for(let t=0;t<Math.min(e.length,15);t++){let n=(e[t]||[]).map(s=>String(s||""));if(n.some(s=>Ft.krw.test(s))&&n.some(s=>Ft.kind.test(s)))return!0}return!1}function bi(e){if(!Array.isArray(e))return null;for(let t=0;t<Math.min(e.length,15);t++){let n=(e[t]||[]).map(i=>String(i||"").replace(/\s+/g,""));if(!n.some(i=>Ft.krw.test(i)))continue;let s={headerRow:t};for(let[i,a]of Object.entries(Ft))s[i]=n.findIndex(o=>a.test(o));return s.amount===s.krw&&(s.amount=n.findIndex((i,a)=>a!==s.krw&&/금액|amount/i.test(i))),s}return null}function _n(e){let t=String(e??"").replace(/\s+/g,"");if(!t)return null;let n=/^[-−–]/.test(t)||/^\(.*\)$/.test(t),s=t.replace(/[^\d]/g,"");if(!s)return null;let i=parseInt(s,10);return isNaN(i)?null:n?-i:i}var _i=[{re:/충전|charge/i,kind:"topup"},{re:/결제\s*취소|payment\s*cancel|취소|cancel|refund/i,kind:"cancel"},{re:/ATM|출금|withdraw/i,kind:"atm"},{re:/기타\s*정산|adjust/i,kind:"adjust"},{re:/결제|payment/i,kind:"payment"}];function Vs(e){let t=String(e||"");for(let n of _i)if(n.re.test(t))return n.kind;return null}function Vt(e,t={}){let n=bi(e);if(!n)return{entries:[],skipped:[]};let s=f=>{let u=0;for(let c=n.headerRow+1;c<Math.min(e.length,n.headerRow+12);c++){let h=e[c]||[];kn(h[n.date+f])&&Vs(h[n.kind+f])&&u++}return u},i=0,a=-1;for(let f of[0,1,-1,2]){let u=s(f);u>a&&(a=u,i=f)}let o={};for(let[f,u]of Object.entries(n))o[f]=f==="headerRow"?u:u>=0?u+i:-1;let r=[],l=[],d=(f,u)=>u>=0?f[u]:void 0;for(let f=o.headerRow+1;f<e.length;f++){let u=e[f]||[],c=u.join(" ").trim(),h=kn(d(u,o.date)),g=Vs(d(u,o.kind));if(!h||!g){c&&l.push(f);continue}let m=Math.abs(_n(d(u,o.krw))||0),y=_n(d(u,o.amount));if(!m){l.push(f);continue}let v=String(d(u,o.currency)||"").replace(/[^A-Z]/gi,"").toUpperCase(),$=String(d(u,o.merchant)||"").replace(/\s+/g," ").trim(),_=_n(d(u,o.balance)),A=g==="cancel"||g==="topup",w="expense",D="travel",x=null,M=!1;g==="topup"?(w="transfer",x="fxTopup",D="etc",$=`트래블월렛 충전${v?` (${v})`:""}`):g==="cancel"?M=!0:g==="atm"&&($=$?`현지 인출 ${$}`:"현지 인출"),$||($=g==="adjust"?"정산":"내역"),w==="expense"&&(D=Ne(z($),t.rules)?be($,"expense",t.rules):"travel"),r.push({type:w,direction:A?"in":"out",date:h,amount:m,transferKind:x,needsCheck:!1,isExtra:!1,category:D,memo:$,normKey:z($),counterpartyRaw:$,balanceAfter:null,foreignBalance:_,accountId:t.accountId||null,paymentMethod:"travelwallet",source:"statement",isRefund:M,installmentMonths:null,isOverseas:!!v&&v!=="KRW",currency:v&&v!=="KRW"?v:"",foreignAmount:y!=null?Math.abs(y):null,twKind:g,confidence:.9,sourceHash:`tw|${h}|${String(d(u,o.time)||"").trim()}|${m}|${$}|${String(d(u,o.approval)||"").trim()}|${g}|${f}`,rawText:c})}return{entries:r,skipped:l}}function ki(e){let t=(e||[]).filter(a=>a&&String(a.str||"").trim()).map(a=>({s:String(a.str),x:+a.x||0,y:Math.round(+a.y||0),w:+a.w||0})).sort((a,o)=>o.y-a.y||a.x-o.x),n=[],s=[],i=null;for(let a of t)i!==null&&Math.abs(a.y-i)>3&&(s.length&&n.push(s),s=[]),s.push(a),i=a.y;return s.length&&n.push(s),n}function wi(e){if(!e.length)return[];let t=[];for(let l=1;l<e.length;l++)t.push(e[l].x-(e[l-1].x+e[l-1].w));let n=t.filter(l=>l>0).sort((l,d)=>l-d),s=n.length?n[Math.floor(n.length/2)]:1,i=Math.max(s*2.5,3),a=[],o=e[0].s,r=e[0].x;for(let l=1;l<e.length;l++)e[l].x-(e[l-1].x+e[l-1].w)>i?(a.push({text:o,x:r}),o=e[l].s,r=e[l].x):o+=e[l].s;return a.push({text:o,x:r}),a}function fa(e){let t=[];for(let r of e||[])for(let l of ki(r))t.push(l);if(!t.length)return[];let n=t.map(r=>wi(r).map(l=>l.text.trim()).filter(l=>l!==""));if(!n.length)return[];let s=new Map;for(let r of n)s.set(r.length,(s.get(r.length)||0)+1);let i=0,a=-1;for(let[r,l]of s)(l>a||l===a&&r>i)&&(a=l,i=r);if(i<3)return n;let o=r=>{let l=String(r).replace(/\s/g,"");return/^-?[\d,]+$/.test(l)&&/\d/.test(l)||/^\d{2,4}[.\-/]\d{1,2}[.\-/]\d{1,2}$/.test(l)||/^\d+$/.test(l)};return n.map(r=>{if(r.length<=i)return r;let l=r.slice();for(;l.length>i;){let d=-1;for(let f=l.length-2;f>=0;f--)if(!o(l[f])&&!o(l[f+1])){d=f;break}if(d<0)break;l.splice(d,2,`${l[d]} ${l[d+1]}`)}return l})}var E=Le.bind(Re);function ha({db:e,patch:t,entries:n,flash:s,onAdopt:i}){let[a,o]=L(!1),[r,l]=L(null),d=U(()=>{let m=new Set(e.fixedExpenses.map(y=>y.normKey));return zs(n).filter(y=>!m.has(y.normKey))},[n,e.fixedExpenses]),f=oe().slice(0,7),u=e.fixedExpenses.filter(m=>m.status!=="ended").map(m=>{let y=n.find(A=>!A.deleted&&A.date.slice(0,7)===f&&(A.fixedId===m.id||Ht(A,m))),v=String(m.dayOfMonth).padStart(2,"0"),$=`${f}-${v}`,_=!y&&$<oe();return{...m,hit:y,due:$,late:_}}).sort((m,y)=>m.dayOfMonth-y.dayOfMonth),c=u.filter(m=>m.hit&&m.hit.status!=="scheduled").reduce((m,y)=>m+y.hit.amount,0),h=u.reduce((m,y)=>m+(y.hit&&y.hit.status!=="scheduled"?y.hit.amount:y.amount),0),g=m=>t(y=>{y.fixedExpenses=y.fixedExpenses.filter(v=>v.id!==m)});return E`
    <div class="fixedWrap">
      ${d.length>0&&E`
        <div class="suggest">
          ...
          <div class="acts">
            <button class="btn primary sm" onClick=${()=>{i(d),s(`고정비 ${d.length}건 등록`)}}>
              전부 고정비로 등록
            </button>
          </div>
        </div>`}
      ...
      <div class="fixedHead">
        <span>이번 달 고정비</span>
        <span class="fixedSum">나감 ₩${F(c)} · 예정 포함 ₩${F(h)}</span>
      </div>

      ${u.length===0&&d.length===0&&E`
      ...
      ${u.map(m=>E`
        <div class=${"fixedRow"+(m.late?" late":"")} key=${m.id}>
          ...
          <button class="del" onClick=${()=>g(m.id)} aria-label="삭제">×</button>
        </div>
    ...
    </div>`}function ga({entries:e,onApply:t,onApplyRules:n,userName:s,ruleCount:i,onClose:a}){let[o,r]=L(!1),[l,d]=L(!1),f=U(()=>{let x=Zs(e,{userName:s});return o?x:x.filter(M=>M.needsReview||M.mixed)},[e,o,s]),[u,c]=L({}),[h,g]=L(!1),[m,y]=L(""),[v,$]=L(null),_=U(()=>ca(m),[m]),A=U(()=>jt(e,_.filter(x=>!x.error)),[e,_]),w=_.filter(x=>!x.error),D=l?f:f.slice(0,40);return E`
    <div class="card">
      ...
      <div class="row wrap" style="margin-top:8px">
        ...
        <button class=${"tag dash"+(h?" on":"")} onClick=${()=>g(!h)}>규칙 한꺼번에 넣기</button>
      </div>
...
oracle = 제외`}></textarea>
          ${_.length>0&&E`
            <div class="ruleList">
              ${_.map((x,M)=>E`
                <div class=${"ruleRow"+(x.error?" bad":"")} key=${M}>
          ...
          <div class="acts">
            <button class="btn primary sm" disabled=${!w.length}
              onClick=${()=>{let x=n(_);$({rules:w.length,changed:x}),y(""),g(!1)}}>
        ...
        ${D.map(x=>E`
          <div class=${"groupRow"+(u[x.normKey]?" settled":"")} key=${x.normKey}>
            ...
            ${x.suggest&&!u[x.normKey]&&E`
              <button class="suggestPill" onClick=${()=>{c(M=>({...M,[x.normKey]:x.suggest})),t(x,x.suggest)}}>
                사람 이름 · 딱 떨어지는 금액 — ${Se.find(M=>M.key===x.suggest)?.label}일까요? 탭하면 정해져요
              </button>`}
            <div class="chips">
              ${Se.filter(M=>M.key!=="income").map(M=>E`
                <button key=${M.key}
                  class=${"chip sm"+((u[x.normKey]||x.category)===M.key?" on":"")}
                  style=${(u[x.normKey]||x.category)===M.key?`background:${M.color};border-color:${M.color}`:`border-color:${M.color}55;color:${M.color}`}
                  onClick=${()=>{c(B=>({...B,[x.normKey]:M.key})),t(x,M.key)}}>${M.label}</button>`)}
            </div>
          </div>`)}
        ${!l&&f.length>40&&E`<button class="pickToggle" onClick=${()=>d(!0)}>나머지 ${f.length-40}곳 더 보기</button>`}
      </div>
      <div class="acts"><button class="btn ghost sm" onClick=${a}>닫기</button></div>
    </div>`}function $a({db:e,entries:t,patch:n,rowProps:s}){let[i,a]=L(!1),[o,r]=L(""),[l,d]=L("bank"),f=e.accounts,u=U(()=>{let y={};for(let v of f){let $=qs(t,v.id);y[v.id]={balance:$.latest?$.latest.balanceAfter:v.balance||0,at:$.latest?$.latest.date:v.balanceAt,gaps:$.gaps}}return y},[f,t]),c=f.reduce((y,v)=>y+(u[v.id]?.balance||0),0),h=U(()=>{let y=oe().slice(0,7);return t.filter(v=>!v.deleted&&v.type==="transfer"&&v.date.slice(0,7)===y).sort((v,$)=>v.date<$.date?1:-1)},[t]),g=y=>f.find(v=>v.id===y)?.name||"?",m={bank:"통장",card:"카드",cash:"현금",fx:"외화",savings:"저축",invest:"투자"};return E`
    <div class="assetWrap">
      <div class="assetTotal">
        <span class="assetTotalLabel">총 자산</span>
        <span class="assetTotalVal">₩${F(c)}</span>
      </div>
      ...
      ${i?E`<div class="card">
            ...
            <div class="chips">
              ${Object.entries(m).map(([y,v])=>E`
                <button key=${y} class=${"tag"+(l===y?" on":"")} onClick=${()=>d(y)}>${v}</button>`)}
            </div>
            <div class="acts">
      ...
      ${h.length>0&&E`
        <div class="transferBox">
          <div class="groupHead">이번 달 계좌 간 이동 ${h.length}건</div>
          ${h.slice(0,10).map(y=>E`
    ...
    </div>`}function va({db:e,entries:t,patch:n,flash:s,onClose:i}){let[a,o]=L(null),r=()=>{let l={accounts:e.accounts,userName:e.settings.userName,settings:e.settings},d=0,f=0,u={};for(let v of t){if(v.deleted||v.transferKind||v.category==="family"||v.category==="travel"||!(v.type==="transfer"||v.type==="expense"&&v.paymentMethod==="account"&&v.direction==="out"))continue;let _=Js(v,l);u[v.id]=_,d++}let{pairs:c,review:h,halves:g}=Xs(t,l);f=c.length;let m=ia(t),y=new Set(m.map(v=>v.refund.id));n(v=>{v.entries=v.entries.map($=>{let _=u[$.id],A=$;($.category==="family"||$.category==="travel")&&$.transferKind&&(A={...A,type:"expense",transferKind:null,updatedAt:Date.now(),dirty:!0}),_&&(A={...A,transferKind:_.transferKind,toAccountId:_.toAccountId||A.toAccountId||null,status:"confirmed",updatedAt:Date.now(),dirty:!0},_.transferKind==="external"?A={...A,type:"expense"}:A.type==="expense"&&(A={...A,type:"transfer"})),y.has($.id)&&(A={...A,type:"expense",isRefund:!0,transferKind:null,status:"confirmed",confidence:1,updatedAt:Date.now(),dirty:!0});let w=c.find(x=>x.inn.id===$.id);w&&(A={...A,deleted:!0,mergedInto:w.out.id,updatedAt:Date.now(),dirty:!0});let D=c.find(x=>x.out.id===$.id);return D&&(A={...A,type:"transfer",transferKind:A.transferKind||"internal",fromAccountId:D.out.accountId||null,toAccountId:D.inn.accountId||null,pairedFrom:[D.out.id,D.inn.id],fee:D.fee||0,updatedAt:Date.now(),dirty:!0}),A})}),o({classified:d,pairedCount:f,review:h.length,halves:g.length,refunds:m.length}),s("정리했어요")};return E`
    <div class="card">
    ...
    </div>`}function ya({db:e,entries:t,onImport:n,onClose:s,flash:i}){let a=ye(null),[o,r]=L(null),[l,d]=L(""),[f,u]=L(e.accounts[0]?.id||""),[c,h]=L(!1),[g,m]=L(!1),y=()=>new Promise((_,A)=>{if(window.XLSX)return _(window.XLSX);let w=document.createElement("script");w.src="./xlsx.js",w.onload=()=>_(window.XLSX),w.onerror=()=>A(new Error("엑셀 읽기 모듈을 불러오지 못했어요")),document.head.appendChild(w)}),v=async()=>{if(window.pdfjsLib)return window.pdfjsLib;let _;try{_=await import(new URL("./pdf.min.mjs",document.baseURI).href)}catch{throw new Error("PDF 읽기 모듈을 불러오지 못했어요. pdf.min.mjs 파일을 올렸는지 확인해주세요")}return _.GlobalWorkerOptions&&(_.GlobalWorkerOptions.workerSrc=new URL("./pdf.worker.min.mjs",document.baseURI).href),window.pdfjsLib=_,_},$=_=>{let A=_.target.files&&_.target.files[0];if(!A)return;d(A.name||"가져오기");let w=/\.(xlsx?|xlsm)$/i.test(A.name),D=/\.pdf$/i.test(A.name)||A.type==="application/pdf",x=new FileReader;x.onload=async()=>{try{let M;if(D){h(!0);let N=await(await v()).getDocument({data:new Uint8Array(x.result)}).promise,W=[];for(let ie=1;ie<=N.numPages;ie++){let $e=await(await N.getPage(ie)).getTextContent();W.push($e.items.map(R=>({str:R.str,x:R.transform[4],y:R.transform[5],w:R.width})))}if(M=fa(W),h(!1),!M.length){i("PDF에서 거래내역을 찾지 못했어요");return}}else if(w){h(!0);let I=await y(),N=I.read(new Uint8Array(x.result),{type:"array"}),W=/거래|날짜|일자|입금|출금|찾으신|맡기신|가맹점|기재내용|적요|잔액|금액|원화금액|종류/,ie=null,$e=-1;for(let O of N.SheetNames){let V=N.Sheets[O];if(!V)continue;let ne=I.utils.sheet_to_json(V,{header:1,raw:!1,defval:""}),xe=ne.slice(0,30).map(Ce=>(Ce||[]).join(" ")).join(" "),re=ne.length+(W.test(xe)?1e4:0);re>$e&&($e=re,ie=O)}let R=N.Sheets[ie||N.SheetNames[0]];M=I.utils.sheet_to_json(R,{header:1,raw:!1,defval:""}),h(!1)}else{let I=sa(new Uint8Array(x.result));M=Qs(I)}let B,H=null;if(Wt(M)){if(B=Vt(M,{rules:e.categoryRules,settings:e.settings}),!B.entries.length){i("트래블월렛 내역을 읽지 못했어요");return}}else{if(H=ea(M),!H){i("어느 칸이 날짜·금액인지 못 찾았어요");return}B=na(M,H,{accountId:g?null:f,paymentMethod:g?"card":"account",rules:e.categoryRules,settings:e.settings})}let j=new Set(t.map(I=>I.sourceHash).filter(Boolean)),X=B.entries.filter(I=>!j.has(I.sourceHash)),J=gn(X,t);r({map:H,rows:M.length,total:B.entries.length,dupFile:B.entries.length-X.length,...J})}catch(M){h(!1),i(`파일을 읽지 못했어요: ${M.message}`)}},x.readAsArrayBuffer(A)};return E`
    <div class="card">
      <div class="cardLabel">명세서 파일 가져오기 (CSV)</div>
      <div class="hint sm">은행·카드사에서 받은 파일을 그대로 넣으세요. 엑셀(.xls, .xlsx), CSV, PDF 모두 됩니다.</div>

      <div class="row wrap">
        <button class=${"tag"+(g?"":" on")} onClick=${()=>m(!1)}>통장</button>
        <button class=${"tag"+(g?" on":"")} onClick=${()=>m(!0)}>카드</button>
        ${!g&&e.accounts.filter(_=>_.kind==="bank").map(_=>E`
          <button key=${_.id} class=${"tag"+(f===_.id?" on":"")} onClick=${()=>u(_.id)}>${_.name}</button>`)}
      </div>

      ${c&&E`<div class="hint sm">파일을 읽는 중이에요… (큰 파일은 조금 걸려요)</div>`}
      ${o&&E`
    ...
    </div>`}var ba=e=>`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}-${String(e.getDate()).padStart(2,"0")}`,An=(e,t,n=0)=>{let s=(i,a)=>{let o=new Date(i+"T00:00:00");return o.setDate(o.getDate()+a),ba(o)};return e.startDate<=s(t.endDate,n)&&t.startDate<=s(e.endDate,n)};function _a({db:e,entries:t,patch:n,flash:s,onClose:i}){let[a,o]=L({}),[r,l]=L({}),[d,f]=L({status:"idle",events:[],msg:""}),u=e.settings&&e.settings.sheetUrl;Ke(()=>{if(!u)return;let w=!0;f({status:"loading",events:[],msg:""});let D=x=>{let M=new Date;return M.setDate(M.getDate()+x),ba(M)};return Fs(u,e.settings.sheetToken,D(-400),D(400)).then(x=>{w&&f({status:"done",events:x,msg:""})}).catch(x=>{w&&f({status:"error",events:[],msg:String(x&&x.message||x)})}),()=>{w=!1}},[u]);let c=U(()=>{let w=oa(t),D=e.trips.filter(I=>I.startDate&&I.endDate),x=D.map(I=>I.startDate),M=I=>Ut(t,I,{allTrips:[...w.trips,...D],settings:e.settings}),B=oe(),H=pa(d.events,D),j=I=>Ut(t,I,{allTrips:D,settings:e.settings,preDays:I.startDate>B?365:120}),X=I=>!H.some(N=>An(N,I,0)),J=ua(t,{settings:e.settings}).filter(I=>!D.some(N=>An(N,I,7))&&!w.trips.some(N=>An(N,I,7)));return{calendar:H.map(j),trips:w.trips.filter(I=>!x.includes(I.startDate)&&X(I)).map(M),singles:w.singles.filter(I=>!x.includes(I.startDate)&&X(I)).map(M),topups:J.filter(X).map(M)}},[t,e.trips,e.settings,d.events]),h=w=>{let D={};for(let x of w.inside)D[x.id]=!0;for(let x of w.booked)D[x.id]=!!x.sure;return D},g=w=>r[w.startDate]||h(w),m=(w,D)=>l(x=>{let M=x[w.startDate]||h(w);return{...x,[w.startDate]:{...M,[D]:!M[D]}}}),y=w=>{let D=g(w);return[...w.inside,...w.booked].filter(x=>D[x.id]&&!x.welfare).reduce((x,M)=>x+(M.isRefund?-M.amount:M.amount),0)},v=w=>[...w.inside,...w.booked].filter(D=>g(w)[D.id]).length,$=w=>{let D=g(w),x=new Set([...w.inside,...w.booked].filter(B=>D[B.id]).map(B=>B.id));if(!x.size&&!w.fromCalendar){s("고른 항목이 없어요");return}let M=Ve();n(B=>{B.trips=[...B.trips,{id:M,name:w.name,startDate:w.startDate,endDate:w.endDate,createdAt:Date.now()}],B.entries=B.entries.map(H=>x.has(H.id)?{...H,tripId:M,updatedAt:Date.now(),dirty:!0}:H)}),s(w.approx?`${w.name} · ${x.size}건을 묶었어요. 여행 탭에서 기간을 실제 일정으로 고치세요`:x.size?`${w.name} · ${x.size}건을 묶었어요`:`${w.name} 여행을 만들었어요. 기간 안의 해외 결제는 들어오는 대로 묶입니다`)},_=(w,D)=>E`
    <div class=${"foundTrip"+(D?" dim":"")} key=${w.startDate}>
      ...
      ${a[w.startDate]&&E`<${xi} trip=${w} picked=${g(w)} toggle=${x=>m(w,x)} />`}
      <div class="acts"><button class="btn primary sm" onClick=${()=>$(w)}>${w.fromCalendar&&!v(w)?"여행 만들기":`${v(w)}건으로 묶기`}</button></div>
    </div>`,A=!c.calendar.length&&!c.trips.length&&!c.singles.length&&!c.topups.length;return E`
    <div class="card">
      ...
      ${d.status==="loading"&&E`<div class="hint sm">구글 캘린더에서 여행 일정을 찾는 중…</div>`}
      ${d.status==="error"&&E`<div class="hint sm">캘린더를 못 읽었어요 · ${d.msg}</div>`}
      ${c.calendar.length>0&&E`
        <div class="foundDivider">구글 캘린더 일정</div>
        ${c.calendar.map(w=>_(w,!1))}`}
      ${c.trips.length>0&&c.calendar.length>0&&E`<div class="foundDivider">결제 기록으로 찾은 여행</div>`}
      ${c.trips.map(w=>_(w,!1))}
      ${c.topups.length>0&&E`
        <div class="foundDivider">충전 기록으로 추정한 여행 (기간은 만든 뒤 고치세요)</div>
        ${c.topups.map(w=>_(w,!0))}`}
      ${c.singles.length>0&&E`
        <div class="foundDivider">이것도 여행인가요? (하루짜리 결제)</div>
        ${c.singles.map(w=>_(w,!0))}`}
      <div class="acts"><button class="btn ghost sm" onClick=${i}>닫기</button></div>
    </div>`}var In=[{key:"trip",label:"여행 기간만",pre:0,post:0},{key:"m4",label:"앞뒤 4개월",pre:120,post:120},{key:"y1",label:"앞뒤 1년",pre:365,post:365},{key:"y2",label:"앞뒤 2년",pre:730,post:730}];function ka({db:e,entries:t,trip:n,patch:s,flash:i,onClose:a}){let o=!!(n&&n.startDate&&n.startDate>oe()),[r,l]=L(o?"y1":"m4"),[d,f]=L(!1),u=In.find(I=>I.key===r)||In[1],c=!n||!n.startDate||!n.endDate,h=U(()=>Ut(t,n,{allTrips:e.trips,preDays:u.pre,postDays:u.post,wide:d,settings:e.settings}),[t,n,e.trips,u,d,e.settings]),g=U(()=>t.filter(I=>n&&I.tripId===n.id),[t,n]),m=U(()=>new Set(h.reimb.map(I=>I.id)),[h]),y=U(()=>{let I=new Map;for(let N of[...g,...h.inside,...h.booked,...h.reimb])I.set(N.id,N);return Array.from(I.values()).sort((N,W)=>N.date<W.date?-1:1)},[g,h]),v=I=>!!I.reimbFrom||m.has(I.id),$=I=>I.transferKind==="welfare",_=y.filter(I=>!v(I)),A=y.filter(v),[w,D]=L(()=>{let I={};for(let N of y)I[N.id]=!!n&&N.tripId===n.id;return I}),x=I=>D(N=>{let W={...N};for(let ie of _)W[ie.id]=I==="all"?!0:I==="none"?!1:ie.date>=n.startDate&&ie.date<=n.endDate;return W}),M=y.filter(I=>w[I.id]),B=M.filter(I=>!$(I)).reduce((I,N)=>I+(v(N)||N.isRefund?-N.amount:N.amount),0),H=M.filter($).reduce((I,N)=>I+(N.isRefund?-N.amount:N.amount),0),j=M.length,X=()=>{let I=Date.now();s(N=>{N.entries=N.entries.map(W=>{if(!(W.id in w))return W;if(!!w[W.id])return m.has(W.id)&&!W.reimbFrom?{...W,tripId:n.id,type:"expense",isRefund:!0,category:"travel",transferKind:null,status:"confirmed",needsCheck:!1,reimbFrom:{type:W.type,transferKind:W.transferKind??null,category:W.category,status:W.status,isRefund:!!W.isRefund},updatedAt:I,dirty:!0}:W.tripId===n.id?W:{...W,tripId:n.id,updatedAt:I,dirty:!0};if(W.tripId!==n.id)return W;if(W.reimbFrom){let{reimbFrom:$e,...R}=W;return{...R,...$e,tripId:null,updatedAt:I,dirty:!0}}return{...W,tripId:null,updatedAt:I,dirty:!0}})}),i(`${j}건으로 정리했어요`),a()},J=I=>E`
    ...
    <div class="card">
      ...
      <div class="row wrap" style="margin-top:6px">
        <button class=${"tag"+(d?" on":"")} onClick=${()=>f(!d)}>
          ${d?"모든 지출 보기":"여행 관련만 보기"}
        </button>
    ...
    </div>`}function wa({db:e,carId:t,patch:n,flash:s,onClose:i,onAddEntry:a}){let[o,r]=L(""),[l,d]=L(null),[f,u]=L({}),[c,h]=L(!0),g=e.maintenance.filter(v=>v.carId===t),m=v=>{let $=ra(v);if(!$.items.length){s("정비 항목을 찾지 못했어요");return}d($);let _={};for(let A of $.items)_[A.label]=!0;u(_)},y=()=>{let v=l.items.filter($=>f[$.label]);if(!v.length){s("고른 항목이 없어요");return}n($=>{$.maintenance=$.maintenance.map(_=>_.carId!==t||!v.find(w=>w.label===_.label)?_:{..._,lastDate:l.date,lastMileage:l.mileage??_.lastMileage});for(let _ of v)$.maintenance.some(A=>A.carId===t&&A.label===_.label)||($.maintenance=[...$.maintenance,{id:Ve(),carId:t,label:_.label,intervalKm:null,intervalMonths:null,lastDate:l.date,lastMileage:l.mileage??null}]);l.mileage&&($.mileageLogs=[{id:Ve(),carId:t,date:l.date,mileage:l.mileage},...$.mileageLogs])}),c&&l.total>0&&a({date:l.date,type:"expense",category:"transport",amount:l.total,memo:`정비 · ${v.map($=>$.label).join(", ")}`.slice(0,40),normKey:z("정비"),carId:t,paymentMethod:"card",source:"receipt",confidence:1}),s(`정비 ${v.length}건을 기록했어요`),r(""),d(null),i()};return E`
    <div class="card">
...
합계 53,000`}
      ...
      ${l&&E`
        ...
        <div class="row wrap" style="margin-top:8px">
          <button class=${"tag"+(c?" on":"")} onClick=${()=>h(!c)}>
            ${c?"지출로도 기록":"이력만 남기기"}
          </button>
          ${c&&l.total>0&&E`<span class="hint sm">₩${F(l.total)}이 이 차량 지출로 들어갑니다</span>`}
        </div>`}
    ...
    </div>`}var he=Le.bind(Re),Ci=[{key:"variable",label:"변동비",color:"#8A6D3B"},{key:"fixed",label:"고정비",color:"#6B5A7A"},{key:"family",label:"가족 송금",color:"#5A7A8A"},{key:"familyExtra",label:"별도 송금",color:"#8A5A3B"},{key:"travel",label:"여행",color:"#3E7A6B"},{key:"cash",label:"현금",color:"#8A8A8A"}],xa=e=>{let t=Math.abs(e);return t>=1e8?`${(e/1e8).toFixed(1)}억`:t>=1e4?`${Math.round(e/1e4).toLocaleString()}만`:F(e)};function Di(e){return e.category==="family"?e.isExtra?"familyExtra":"family":e.category==="travel"?"travel":e.fixedId?"fixed":e.paymentMethod==="cash"?"cash":"variable"}function Mn(e){return!(e.deleted||e.type!=="expense"||e.transferKind&&e.transferKind!=="external")}var Rn=e=>e.isRefund?-e.amount:e.amount;function Si({entries:e,range:t}){let n=U(()=>{let a={};for(let l of e){if(!Mn(l))continue;let d=Di(l);a[d]=(a[d]||0)+Rn(l)}let o=Ci.map(l=>({...l,value:a[l.key]||0})).filter(l=>l.value>0),r=o.reduce((l,d)=>l+d.value,0);return{rows:o.sort((l,d)=>d.value-l.value),total:r}},[e]);if(!n.total)return he`<div class="empty">이 기간 지출이 없어요.</div>`;let s=0,i=n.rows.map(a=>{let o=s;return s+=a.value/n.total,{...a,from:o,to:s}});return he`
    <div class="chartWrap">
      <div class="donutRow">
    ...
    </div>`}function Ai({all:e,months:t=6}){let n=U(()=>{let i=oe().slice(0,7),a=[],[o,r]=i.split("-").map(Number);for(let u=t-1;u>=0;u--){let c=new Date(o,r-1-u,1);a.push(`${c.getFullYear()}-${String(c.getMonth()+1).padStart(2,"0")}`)}let l=Object.fromEntries(a.map(u=>[u,{income:0,expense:0}]));for(let u of e){let c=u.date.slice(0,7);l[c]&&(u.type==="income"&&!u.deleted?l[c].income+=u.amount:Mn(u)&&(l[c].expense+=Rn(u)))}let d=a.map(u=>({key:u,label:u.slice(5)+"월",...l[u]})),f=Math.max(1,...d.map(u=>Math.max(u.income,u.expense)));return{rows:d,peak:f}},[e,t]),s=96;return he`
    <div class="chartWrap">
      <div class="barChart">
    ...
    </div>`}var ge=Le.bind(Re),Sa=[{key:"variable",label:"변동비",color:"#8A6D3B"},{key:"fixed",label:"고정비",color:"#6B5A7A"},{key:"family",label:"가족 송금",color:"#5A7A8A"},{key:"familyExtra",label:"별도 송금",color:"#8A5A3B"},{key:"travel",label:"여행",color:"#3E7A6B"},{key:"cash",label:"현금",color:"#8A8A8A"}],Mi=e=>!e.deleted&&e.type==="expense"&&(!e.transferKind||e.transferKind==="external"),Da=e=>e.isRefund?-e.amount:e.amount;function Ri(e){return e.category==="family"?e.isExtra?"familyExtra":"family":e.category==="travel"?"travel":e.fixedId?"fixed":e.paymentMethod==="cash"?"cash":"variable"}function Kn(e,t,n){let s=e.filter(f=>f.date>=t&&f.date<=n),i={},a={},o={},r=0,l=0,d=0;for(let f of s){if(f.type==="income"&&!f.deleted){r+=f.amount;continue}if(f.deleted)continue;if(f.transferKind==="welfare"){d+=Da(f);continue}if(!Mi(f))continue;let u=Da(f);l+=u;let c=Ri(f);i[c]=(i[c]||0)+u,a[f.category]=(a[f.category]||0)+u;let h=f.normKey||f.memo||"";h&&(o[h]||(o[h]={name:f.memo,total:0,count:0}),o[h].total+=u,o[h].count+=1)}return{from:t,to:n,income:r,expense:l,welfare:d,axes:i,cats:a,merchants:o,count:s.length}}var Aa=(e,t)=>t?Math.round((e-t)/t*100):e?null:0,Tn=({now:e,before:t,invert:n})=>{if(t==null)return null;let s=e-t;if(!s)return ge`<span class="dl same">변화 없음</span>`;let i=s>0,a=n?!i:i,o=Aa(e,t);return ge`
    <span class=${"dl "+(a?"bad":"good")}>
      ${i?"▲":"▼"} ${F(Math.abs(s))}${o!==null?` (${Math.abs(o)}%)`:""}
    </span>`},En=(e,t)=>new Date(e,t,0).getDate(),Fe=e=>String(e).padStart(2,"0");function Ki(e,t){let[n,s]=t.split("-").map(Number);if(e==="month"){let o={from:`${t}-01`,to:`${t}-${Fe(En(n,s))}`,label:`${n}년 ${s}월`},r=new Date(n,s-2,1),l=r.getFullYear(),d=r.getMonth()+1,f={from:`${l}-${Fe(d)}-01`,to:`${l}-${Fe(d)}-${Fe(En(l,d))}`,label:`${d}월`},u={from:`${n-1}-${Fe(s)}-01`,to:`${n-1}-${Fe(s)}-${Fe(En(n-1,s))}`,label:`작년 ${s}월`};return{cur:o,prev:f,lastYear:u}}let i={from:`${n}-01-01`,to:`${n}-12-31`,label:`${n}년`},a={from:`${n-1}-01-01`,to:`${n-1}-12-31`,label:`${n-1}년`};return{cur:i,prev:a,lastYear:null}}function Ti(e,t,n){let s=[];if(!t||!t.count)return s;for(let r of Sa){let l=e.axes[r.key]||0,d=t.axes[r.key]||0,f=l-d;if(Math.abs(f)<1e5)continue;let u=Aa(l,d);d===0&&l>0?s.push({kind:"new",text:`${r.label}가 새로 ${F(l)}원 발생했습니다`}):l===0&&d>0?s.push({kind:"gone",text:`${r.label} ${F(d)}원이 이번에는 없습니다`}):u!==null&&Math.abs(u)>=20&&s.push({kind:f>0?"up":"down",text:`${r.label}가 ${n}보다 ${F(Math.abs(f))}원 ${f>0?"늘었습니다":"줄었습니다"} (${Math.abs(u)}%)`})}let i=[],a=new Set([...Object.keys(e.cats),...Object.keys(t.cats)]);for(let r of a){let l=(e.cats[r]||0)-(t.cats[r]||0);Math.abs(l)>=15e4&&i.push({k:r,d:l})}i.sort((r,l)=>Math.abs(l.d)-Math.abs(r.d));for(let r of i.slice(0,3)){let l=ze[r.k]?.label||r.k;s.push({kind:r.d>0?"up":"down",text:`${l}가 ${F(Math.abs(r.d))}원 ${r.d>0?"늘었습니다":"줄었습니다"}`})}for(let[r,l]of Object.entries(e.merchants))t.merchants[r]||l.total<3e5||s.push({kind:"new",text:`${l.name}에 ${F(l.total)}원을 썼습니다. ${n}에는 없던 지출입니다`});let o=e.income-e.expense;return e.income>0&&s.push({kind:o<0?"warn":"ok",text:o<0?`수입보다 ${F(-o)}원을 더 썼습니다`:`${F(o)}원이 남았습니다`}),s.slice(0,8)}function Ia({all:e,monthStartDay:t}){let[n,s]=L("month"),[i,a]=L(()=>oe().slice(0,7)),o=U(()=>Ki(n,i),[n,i]),r=U(()=>Kn(e,o.cur.from,o.cur.to),[e,o]),l=U(()=>Kn(e,o.prev.from,o.prev.to),[e,o]),d=U(()=>o.lastYear?Kn(e,o.lastYear.from,o.lastYear.to):null,[e,o]),f=l.count?l:d&&d.count?d:null,u=l.count?o.prev.label:d&&d.count?o.lastYear.label:"",c=U(()=>Ti(r,f,u),[r,f,u]),h=$=>{if(n==="year"){a(`${Number(i.slice(0,4))+$}-01`);return}let[_,A]=i.split("-").map(Number),w=new Date(_,A-1+$,1);a(`${w.getFullYear()}-${Fe(w.getMonth()+1)}`)},g=Sa.map($=>({...$,now:r.axes[$.key]||0,before:f?f.axes[$.key]||0:null})).filter($=>$.now>0||($.before||0)>0),m=Math.max(1,...g.map($=>Math.max($.now,$.before||0))),y=()=>{let $=[];$.push(`${o.cur.label} 결산`),$.push(""),$.push(`수입   ${F(r.income)}원`),$.push(`지출   ${F(r.expense)}원`),$.push(`남은돈 ${F(r.income-r.expense)}원`),$.push("");for(let A of g)$.push(`${A.label.padEnd(8)} ${F(A.now)}원`);if(c.length){$.push(""),$.push(`[${u} 대비]`);for(let A of c)$.push(`· ${A.text}`)}let _=$.join(`
`);try{navigator.clipboard.writeText(_)}catch{window.prompt("복사하세요",_)}},v=r.income-r.expense;return ge`
    <div class="report">
      ...
      <div class="repHead">
        <button class="nav" onClick=${()=>h(-1)} aria-label="이전">‹</button>
        <span class="repTitle">${o.cur.label} 결산</span>
        <button class="nav" onClick=${()=>h(1)} aria-label="다음">›</button>
      </div>
  ...
  <svg viewBox="0 0 24 24" width=${n.size||16} height=${n.size||16} fill="none"
    ...
    <div class="form">
      ...
      <div class="chips">
        ...
        ${d?b`<span class="inline">
              <input class="inp mini" autofocus placeholder="여행 이름" value=${u}
                onInput=${$=>c($.target.value)}
                onKeyDown=${$=>{$.key==="Enter"&&u.trim()&&(v({tripId:r(u.trim(),e.date).id}),c(""),f(!1))}} />
              <button class="round" onClick=${()=>{u.trim()&&(v({tripId:r(u.trim(),e.date).id}),c(""),f(!1))}}><${Yt} size=${13} /></button>
      ...
      <div class="chips">
        ...
        ${h?b`<span class="inline">
              <input class="inp mini" autofocus placeholder="차량 이름" value=${m}
                onInput=${$=>y($.target.value)}
                onKeyDown=${$=>{$.key==="Enter"&&m.trim()&&(v({carId:l(m.trim()).id}),y(""),g(!1))}} />
              <button class="round" onClick=${()=>{m.trim()&&(v({carId:l(m.trim()).id}),y(""),g(!1))}}><${Yt} size=${13} /></button>
    ...
    </div>`}function qi({entry:e,trips:t,vehicles:n,onQuickCat:s,onEdit:i,onDelete:a,selecting:o,checked:r,onToggle:l}){let[d,f]=L(!1),u=ye(null),c=ye(!1),h=()=>{c.current=!1,u.current=setTimeout(()=>{u.current=null,i(e)},550)},g=()=>{u.current&&(clearTimeout(u.current),u.current=null,c.current||f(_=>!_))},m=()=>{c.current=!0,u.current&&(clearTimeout(u.current),u.current=null)},y=we[e.category],v=e.tripId&&t.find(_=>_.id===e.tripId),$=e.carId&&n.find(_=>_.id===e.carId);return o?b`
      <div class="rowWrap">
    ...
    <div class="rowWrap">
      <div class=${"entry"+(e.status==="pending"?" pending":"")}
        onPointerDown=${h} onPointerUp=${g} onPointerLeave=${m} onPointerCancel=${m}>
...
또는 통장 거래내역 여러 줄을 통째로`}></textarea>
      ...
      ${i&&b`
        <div class="card">
          ...
          <${Ma} value=${i} onChange=${a} onSave=${Za} onCancel=${()=>a(null)}
      ...
      ${u&&b`
        <div class="card">
          ...
          <${Ma} value=${u} onChange=${c}
            ...
            onCancel=${()=>c(null)} saveLabel="수정 완료"
      ...
      <div class="tabs">
        <button class=${"tab"+(h==="month"?" on":"")} onClick=${()=>g("month")}>월별</button>
        <button class=${"tab"+(h==="asset"?" on":"")} onClick=${()=>g("asset")}>자산</button>
        <button class=${"tab"+(h==="trip"?" on":"")} onClick=${()=>{g("trip"),!B&&e.trips.length&&H(e.trips[0].id)}}><${On} size=${12} /> 여행</button>
        <button class=${"tab"+(h==="car"?" on":"")} onClick=${()=>{g("car"),!j&&e.vehicles.length&&X(e.vehicles[0].id)}}><${Pn} size=${12} /> 차량</button>
      </div>
      ...
      ${m.trim()?b`<div class="results">
          ...
          ${Ie.length===0?b`<div class="empty">이 기간 기록이 아직 없어요.<br />위에 문자나 거래내역을 붙여넣어 보세요.</div>`:b`
              ...
              <${qt} list=${Ie} ...${at} />`}`:h==="asset"?b`<${$a} db=${e} entries=${Z} patch=${ee} rowProps=${at} />`:h==="trip"?b`
          <div class="toolRow">
    ...
    </div>`}function Qi(e){let t=String(e||""),n=t.charCodeAt(t.length-1)-44032;if(n<0||n>11171)return"로";let s=n%28;return s===0||s===8?"로":"으로"}function er({db:e,live:t,tripId:n,setTripId:s,rowProps:i,patch:a,flash:o}){let[r,l]=L(!1),[d,f]=L({name:"",startDate:Ae(),endDate:Ae()}),[u,c]=L(null),[h,g]=L(!1),m=()=>{let D=e.trips.find(M=>M.id===n);if(!D)return;let x=t.filter(M=>M.tripId===D.id).length;a(M=>{M.trips=M.trips.filter(B=>B.id!==D.id),M.entries=M.entries.map(B=>B.tripId===D.id?{...B,tripId:null,updatedAt:Date.now(),dirty:!0}:B)}),g(!1),c(null),s(null),o(x>0?`${D.name} 여행을 지웠어요 · ${x}건은 기록으로 남았어요`:`${D.name} 여행을 지웠어요`)},y=e.trips.map(D=>{let x=xn(t,D);return{...D,expense:x.total,welfare:x.welfare}}),v=y.find(D=>D.id===n),$=t.filter(D=>D.tripId===n).sort((D,x)=>D.date<x.date?1:D.date>x.date?-1:x.createdAt-D.createdAt),_=v&&(!v.startDate||!v.endDate),A=u||(_?{id:v.id,startDate:"",endDate:"",name:v.name}:null),w=()=>{!A.startDate||!A.endDate||A.startDate>A.endDate||(a(D=>{D.trips=D.trips.map(x=>x.id===A.id?{...x,name:(A.name||x.name).trim(),startDate:A.startDate,endDate:A.endDate}:x)}),c(null),o("여행 기간을 고쳤어요. '항목 조정'에서 지출을 넣으세요"))};return b`
    <div class="cards">
      ${y.length===0&&b`<div class="empty">아직 만든 여행이 없어요.<br />아래 "+ 여행 추가"로 기간을 넣어 만들면 그 기간 지출을 찾아 묶어줍니다.</div>`}
      ${y.map(D=>b`
        <button key=${D.id} class=${"sumCard"+(n===D.id?" on":"")} onClick=${()=>{s(D.id),c(null)}}>
    ...
    ${r&&b`
      <div class="card">
        ...
        <input class="inp" placeholder="여행 이름 (예: 오키나와 26.03)" value=${d.name}
          onInput=${D=>f(x=>({...x,name:D.target.value}))} />
        <div class="row wrap">
          <input type="date" class="inp date" value=${d.startDate} onInput=${D=>f(x=>({...x,startDate:D.target.value}))} />
          <span class="hint sm">~</span>
          <input type="date" class="inp date" value=${d.endDate} onInput=${D=>f(x=>({...x,endDate:D.target.value}))} />
        </div>
        <div class="acts">
          <button class="btn ghost sm" onClick=${()=>l(!1)}>취소</button>
          <button class="btn primary sm" disabled=${!d.name.trim()||d.startDate>d.endDate}
            onClick=${()=>{let D=ke();a(x=>{x.trips=[...x.trips,{id:D,name:d.name.trim(),startDate:d.startDate,endDate:d.endDate,createdAt:Date.now()}]}),s(D),l(!1),f({name:"",startDate:Ae(),endDate:Ae()}),o("여행을 만들었어요. '항목 조정'으로 지출을 넣으세요")}}>만들기</button>
        </div>
      </div>`}
    ${v&&b`
      <div class="monthBar">
      ...
      ${A?b`
          <div class="card tripDates">
            ...
            <input class="inp" placeholder="여행 이름" value=${A.name} onInput=${D=>c({...A,name:D.target.value})} />
            <div class="row wrap">
              <input type="date" class="inp date" value=${A.startDate} onInput=${D=>c({...A,startDate:D.target.value})} />
              <span class="hint sm">~</span>
              <input type="date" class="inp date" value=${A.endDate} onInput=${D=>c({...A,endDate:D.target.value})} />
            </div>
            <div class="acts">
              ${u&&!_&&b`<button class="btn ghost sm" onClick=${()=>c(null)}>취소</button>`}
      ...
      ${(()=>{let D=xn(t,v);return b`
          <div class="tripSum">
            <div class="tripSumTop">
              ...
              ${!_&&!u&&b`<button class="pickToggle" onClick=${()=>c({id:v.id,name:v.name,startDate:v.startDate,endDate:v.endDate})}>기간 고치기</button>`}
              ${!u&&!h&&b`<button class="pickToggle warn" onClick=${()=>g(!0)}>여행 삭제</button>`}
            </div>
            ${h&&b`
              <div class="delAsk">
            ...
            ${Object.keys(D.fx).length>0&&b`
              <div class="tripFx">${Object.entries(D.fx).map(([x,M])=>`${x} ${M.toLocaleString()}`).join(" · ")}</div>`}
          </div>`})()}
      ${$.length===0?b`<div class="empty">이 여행에 묶인 내역이 아직 없어요.<br />위의 <b>항목 조정</b>에서 항공권·숙박·현지 결제를 넣으세요.</div>`:b`<${qt} list=${$} ...${i} />`}`}`}var Ra=[{key:"y1",label:"최근 1년",days:365},{key:"y2",label:"최근 2년",days:730},{key:"all",label:"전체",days:null}];function tr({db:e,live:t,carId:n,setCarId:s,car:i,carKm:a,mileage:o,setMileage:r,mileageDate:l,setMileageDate:d,maintEdit:f,setMaintEdit:u,patch:c,addVehicle:h,latestMileage:g,rowProps:m,assignCar:y}){let[v,$]=L(!1),[_,A]=L(""),[w,D]=L("all"),[x,M]=L("gasoline"),B=U(()=>{let R=Ra.find(V=>V.key===w);if(!R||!R.days)return"0000-00-00";let O=new Date;return O.setDate(O.getDate()-R.days),dt(O)},[w]),H=R=>R.date>=B,j=e.vehicles.map(R=>{let O=0;for(let V of t)V.carId===R.id&&V.type==="expense"&&H(V)&&(O+=V.isRefund?-V.amount:V.amount);return{...R,expense:O}}),X=e.maintenance.filter(R=>R.carId===n),J=e.mileageLogs.filter(R=>R.carId===n).sort((R,O)=>R.date<O.date?1:-1).slice(0,5),I=t.filter(R=>R.carId===n&&H(R)).sort((R,O)=>R.date<O.date?1:R.date>O.date?-1:O.createdAt-R.createdAt),N=U(()=>{let R=new Map;for(let O of I){if(O.type!=="expense")continue;let V=O.category||"etc";R.set(V,(R.get(V)||0)+(O.isRefund?-O.amount:O.amount))}return Array.from(R.entries()).filter(([,O])=>O!==0).sort((O,V)=>V[1]-O[1])},[I]),W=N.reduce((R,[,O])=>R+O,0),ie=U(()=>t.filter(R=>!R.carId&&R.type==="expense"&&R.category==="car"&&H(R)),[t]),$e=(R,O)=>c(V=>{V.maintenance=V.maintenance.map(ne=>ne.id===R?{...ne,...O}:ne)});return b`
    <div class="cards">
      ${j.length===0&&b`<div class="empty">아직 등록한 차량이 없어요.</div>`}
      ${j.map(R=>b`
        <button key=${R.id} class=${"sumCard"+(n===R.id?" on":"")} onClick=${()=>s(R.id)}>
          <div class="sumName"><${Pn} size=${12} /> ${R.name}</div>
    ...
    ${v&&b`
      <div class="card">
        ...
        <input class="inp" placeholder="차량 이름 (예: 아반떼)" value=${_} onInput=${R=>A(R.target.value)} />
        <div class="miniLabel">연료</div>
        <div class="chips">
          ${Xt.map(R=>b`
            <button key=${R.key} class=${"tag"+(x===R.key?" on":"")} onClick=${()=>M(R.key)}>${R.label}</button>`)}
        </div>
        <div class="hint sm">
        ...
        <div class="acts">
          ...
          <button class="btn primary sm" disabled=${!_.trim()}
            onClick=${()=>{s(h(_.trim(),x).id),A(""),$(!1)}}>만들기</button>
        </div>
      </div>`}

    <div class="row wrap" style="margin-top:10px">
      ${Ra.map(R=>b`
        <button key=${R.key} class=${"tag"+(w===R.key?" on":"")} onClick=${()=>D(R.key)}>${R.label}</button>`)}
    </div>

    ${i&&(()=>{let R=j.find(ne=>ne.id===n)?.expense||0,O=I.map(ne=>ne.date).sort(),V=O.length?Math.max(1,Math.round((new Date(O[O.length-1])-new Date(O[0]))/2592e6)+1):1;return b`
      <div class="monthBar">
        <span class="monthLabel">${i.name}${i.fuel?b`<span class="fuelTag">${(Xt.find(ne=>ne.key===i.fuel)||{}).label||""}</span>`:""}</span>
        <span class="totals"><span class="exp">유지비 ₩${q(R)}</span></span>
      </div>
      ${R>0&&b`
        <div class="carSpan">
          ${O.length?`${O[0]} ~ ${O[O.length-1]} · ${I.length}건`:""}
          ${V>1?` · 월평균 ₩${q(Math.round(R/V))}`:""}
          ${V>=6?` · 연환산 ₩${q(Math.round(R/V*12))}`:""}
        </div>`}
      `})()}
    ${i&&b`
      ...
      <div class="box">
        ...
        <div class="row wrap">
          <input type="number" class="inp" placeholder="계기판 거리(km)" value=${o} onInput=${R=>r(R.target.value)} />
          <input type="date" class="inp date" value=${l} onInput=${R=>d(R.target.value)} />
          <button class="btn primary sm" disabled=${!o}
            onClick=${()=>{c(R=>{R.mileageLogs=[{id:ke(),carId:n,date:l,mileage:parseInt(o,10)},...R.mileageLogs]}),r("")}}>
            <${Yt} size=${14} /> 기록</button>
        </div>
        ${J.length>0&&b`<div class="logList">${J.map(R=>b`<span class="log" key=${R.id}>${ut(R.date)} · ${R.mileage.toLocaleString("ko-KR")}km</span>`)}</div>`}
      </div>

      <div class="maintList">
        ${X.map(R=>{let{status:O,remKm:V,remMonths:ne}=Pi(R,a),xe=f&&f.id===R.id;return b`
            <div class="box maint" key=${R.id}>
              <div class="boxTop">
                <span class="maintName"><${Wi} size=${13} /> ${R.label}</span>
              ...
              ${xe?b`<div class="row wrap">
                    ...
                    <div class="acts">
                      ...
                      <button class="btn primary sm" onClick=${()=>{$e(R.id,f),u(null)}}>저장</button>
                    </div>
                  </div>`:b`<div class="acts">
                    <button class="btn ghost sm" onClick=${()=>u({...R})}>수정</button>
                    <button class="btn primary sm" onClick=${()=>$e(R.id,{lastDate:Ae(),lastMileage:g(n)})}>오늘 처리함</button>
                  </div>`}
            </div>`})}
      </div>
      ...
      ${N.length>0&&b`
        <div class="box">
          ...
          <div class="carBreak">
            ${N.map(([R,O])=>b`
              <div class="carBreakRow" key=${R}>
                <span class="dot" style=${`background:${(we[R]||{}).color||"#999"}`}></span>
                <span class="carBreakName">${(we[R]||{}).label||"기타"}</span>
                <span class="carBreakBar"><i style=${`width:${Math.max(2,Math.round(O/W*100))}%;background:${(we[R]||{}).color||"#999"}`}></i></span>
      ...
      ${ie.length>0&&e.vehicles.length>0&&b`
        <div class="box">
          ...
          <div class="unassignList">
            ${ie.slice(0,6).map(R=>b`
              <div class="unassignRow" key=${R.id}>
                <span class="unassignMemo">${R.memo}</span>
                <span class="unassignAmt">₩${q(R.amount)}</span>
                <div class="chips">
                  ${e.vehicles.map(O=>b`
                    <button key=${O.id} class="chip sm" onClick=${()=>y(R,O.id)}>${O.name}</button>`)}
                </div>
              </div>`)}
          </div>
        </div>`}
    ...
    </div>`}function nr({db:e,setDb:t,onClose:n,flash:s,onSync:i,onUndoImport:a}){let o=ye(null),[r,l]=L(!1),d=U(()=>JSON.stringify(e).length,[e]),f=()=>{let c=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),h=URL.createObjectURL(c),g=document.createElement("a");g.href=h,g.download=`가계부-백업-${Ae()}.json`,document.body.appendChild(g),g.click(),document.body.removeChild(g),setTimeout(()=>URL.revokeObjectURL(h),1e3)},u=c=>{let h=c.target.files&&c.target.files[0];if(!h)return;let g=new FileReader;g.onload=()=>{try{let m=JSON.parse(g.result);if(!m.entries)throw new Error("형식이 다릅니다");t({...Te,...m,settings:{...Te.settings,...m.settings||{}}}),s(`${m.entries.length}건을 불러왔어요`),n()}catch{s("파일을 읽지 못했어요")}},g.readAsText(h)};return b`
    <div class="card settings">
      ...
      <label class="setRow">
        ...
        <input class="inp" placeholder="이체 판정에 씁니다" value=${e.settings.userName}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,userName:c.target.value}}))} />
      </label>

      <label class="setRow">
        <span>월 시작일</span>
        <input class="inp" type="number" min="1" max="28" value=${e.settings.monthStartDay}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,monthStartDay:Math.min(28,Math.max(1,parseInt(c.target.value||"1",10)))}}))} />
      </label>
      ...
      <${ct} label="수입처" items=${e.settings.incomeSources||[]} placeholder="예: 근무하는 회사 이름"
        onChange=${c=>t(h=>({...h,settings:{...h.settings,incomeSources:c}}))} />
      <${ct} label="항상 제외" items=${e.settings.excludedCounterparties||[]} placeholder="예: 어머니 성함"
        onChange=${c=>t(h=>({...h,settings:{...h.settings,excludedCounterparties:c}}))} />
      ...
      <${ct} label="복지카드" items=${e.settings.welfareCards||[]} placeholder="예: 현대카드"
        onChange=${c=>t(h=>({...h,settings:{...h.settings,welfareCards:c}}))} />
      <label class="setRow">
        <span>복지 한도</span>
        <input class="inp" type="number" inputmode="numeric" placeholder="연간 한도 (예: 1200000)"
          value=${e.settings.welfareLimit||""}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,welfareLimit:parseInt(c.target.value||"0",10)}}))} />
      </label>
      <label class="setRow">
        <span>시작 시점</span>
        <input class="inp" placeholder="예: 2026-01" value=${e.settings.welfarePeriodStart||""}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,welfarePeriodStart:c.target.value.trim()}}))} />
      </label>
      ...
      <${ct} label="가족 송금" items=${e.settings.familyTransferKeywords||[]} placeholder="예: 하나머니충전"
        onChange=${c=>t(h=>({...h,settings:{...h.settings,familyTransferKeywords:c}}))} />
      <label class="setRow">
        <span>생활비 최소</span>
        <input class="inp" type="number" inputmode="numeric" placeholder="예: 800000"
          value=${e.settings.familyAmountMin||""}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,familyAmountMin:parseInt(c.target.value||"0",10)}}))} />
      </label>
      <label class="setRow">
        <span>생활비 최대</span>
        <input class="inp" type="number" inputmode="numeric" placeholder="예: 1400000"
          value=${e.settings.familyAmountMax||""}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,familyAmountMax:parseInt(c.target.value||"0",10)}}))} />
      </label>
      ...
      <label class="setRow">
        ...
        <input type="checkbox" checked=${e.settings.familyExtraExcluded!==!1}
          onChange=${c=>t(h=>({...h,settings:{...h.settings,familyExtraExcluded:c.target.checked}}))} />
      </label>
      <${ct} label="여행 자금" items=${e.settings.travelKeywords||[]} placeholder="예: 트래블월렛"
        onChange=${c=>t(h=>({...h,settings:{...h.settings,travelKeywords:c}}))} />

      <div class="setDivider">구글시트 연동</div>
      <label class="setRow">
        <span>시트 주소</span>
        <input class="inp" placeholder="https://script.google.com/.../exec" value=${e.settings.sheetUrl}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,sheetUrl:c.target.value.trim()}}))} />
      </label>
      <label class="setRow">
        <span>연결 암호</span>
        <input class="inp" placeholder="Apps Script에 적은 값과 같게" value=${e.settings.sheetToken}
          onInput=${c=>t(h=>({...h,settings:{...h.settings,sheetToken:c.target.value.trim()}}))} />
      </label>
      <div class="acts">
        <button class="btn ghost sm" onClick=${async()=>{try{await Hs(e.settings.sheetUrl,e.settings.sheetToken),s("연결됐어요")}catch(c){s(`연결 실패: ${c.message}`)}}} disabled=${!e.settings.sheetUrl}>연결 확인</button>
      ...
      <div class="setStat">버전 <b>v29</b> · 기록 ${e.entries.filter(c=>!c.deleted).length}건 · 분류 규칙 ${(e.categoryRules||[]).length}개 · 보낼 것 ${e.entries.filter(c=>c.dirty).length}건 · 저장 용량 ${(d/1024).toFixed(0)}KB</div>
      ...
      ${r?b`
          <div class="dangerBox">
            <div>기록 ${e.entries.filter(c=>!c.deleted).length}건, 여행 ${e.trips.length}건, 고정비 ${e.fixedExpenses.length}건이 사라집니다.</div>
            ...
            <div class="acts">
              ...
              <button class="btn danger sm" onClick=${()=>{t(c=>({...Te,categoryRules:c.categoryRules||[],settings:{...c.settings,lastSyncAt:""}})),l(!1),s("기록을 지웠어요"),n()}}>정말 지우기</button>
            </div>
          </div>`:b`
          <div class="acts">
      ...
      ${(e.imports||[]).length>0&&b`
        ...
        <div class="importList">
          ${(e.imports||[]).map(c=>b`
            <div class=${"importRow"+(c.undone?" undone":"")} key=${c.id}>
              <div class="importMain">
                <div class="importName">${c.name}</div>
                <div class="sub">${new Date(c.at).toLocaleDateString("ko-KR")} · ${c.count}건${c.undone?" · 되돌림":""}</div>
              </div>
              ${!c.undone&&b`<button class="btn ghost sm" onClick=${()=>a(c)}>되돌리기</button>`}
            </div>`)}
        </div>`}
